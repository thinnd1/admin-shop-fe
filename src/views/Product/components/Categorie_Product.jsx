...
function CategorieProduct({ HandleCategoriasation }) {
  ...
  const productCategories = ExtractData(
    "http://127.0.0.1:8000/api/admin/product/category/index",
    SetCategories
  );
  const productBrands = ExtractData(
    "http://127.0.0.1:8000/api/product/brands/index",
    SetBrands
  );
              ...
              {Categories.map((Category, id) => (
                <div className="flex items-center" key={id}>
                  <input
                    ...
                    onClick={(event) =>
                      HandleCategoriasation(event.target.value)
                    }
                  />
            ...
            <div className="space-y-2">
              {brands.map((brand) => (
                <div className="flex items-center" key={brand.id}>
                  <input
                    type="checkbox"
                    name={`brand-${brand.id}`} // Use a unique identifier for the name attribute
                    id={`brand-${brand.id}`} // Use a unique identifier for the id attribute
                    className="text-primary focus:ring-0 rounded-sm cursor-pointer"
                  />
                  <label
