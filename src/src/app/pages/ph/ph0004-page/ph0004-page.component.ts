...
@Component({
  selector: 'app-ph0004-page',
  templateUrl: './ph0004-page.component.html',
  styleUrls: ['./ph0004-page.component.scss']
})
export class Ph0004PageComponent implements OnInit, AfterViewInit, OnDestroy {
  numTab: any;
  ...
  const: any = {};
  listDrugstore = [];
  selectDrugstore2: any= {};
  selectDrugstore3: any= {};
  latestPage = [];
  allPage = [];
  keyword = '';
  options: any;
  ...
  constructor(private router: Router,
    ...
    this.pharmacyService.getListDrugStore(this.userSession.officeId).subscribe(
      (settings) => {
        this.dialogService.setLoaderVisible(false);
        this.listDrugstore = [{id: '0', text: this.translate.instant('PH0015.MEDICAL')}];
        const listDrugstore = settings.drugStores;
        for (let i = 0; i < listDrugstore.length; i++) {
          const drug = {'id': listDrugstore[i].id, 'text': listDrugstore[i].name, 'nameKana': listDrugstore[i].nameKana};
          this.listDrugstore.push(drug);
        }
        if (this.listDrugstore) {
          this.selectDrugstore2 = this.listDrugstore[0];
          this.selectDrugstore3 = this.listDrugstore[0];
        }
      },
      (error) => {
        this.dialogService.setLoaderVisible(false);
  ...
  callApi(replace) {
    switch (this.numTab) {
      case 1:
        break;
      case 2:
        this.formSearchTab2.officeId = this.selectDrugstore2.id && this.selectDrugstore2.id !== '0' ? this.selectDrugstore2.id : '';
        if (replace) {
        ...
        break;
      case 3:
        this.formSearchTab3.officeId = this.selectDrugstore3.id && this.selectDrugstore3.id !== '0' ? this.selectDrugstore3.id : '';
        if (replace) {
  ...
  getUpdateCSV(dataList) {
    ...
    const header = [this.msgStore['PH0004'].UPDATED, this.msgStore['PH0004'].OFFICE_ID,
      this.msgStore['PHARMACY.TRACING_REPORT'].PHARMACIST_NAME,
    ...
    for (let i = 0; i <  dataList.length; i++) {
      ...
      data[1] = dataList[i]['drugStoreOfficeName'];
      data[2] = dataList[i]['pharmacistName']; // TODO
      data[3] = dataList[i]['patientCode'] ? dataList[i]['patientCode'] : '';
      data[4] = dataList[i]['patientName'] ? dataList[i]['patientName'] : '';
      data[5] = moment(dataList[i]['prescriptionIssueDate']).format('YYYY/MM/DD');
      if ( dataList[i].prescriptionUpdateReason !== -1) {
        data[6] = dataList[i]['prescriptionUpdateReason'] ?
      ...
      if ( dataList[i]['protocolUsage'] !== -1) {
        data[7] = dataList[i]['protocolUsage'] ? this.msgStore['PH0015'].PROTOCOL_YES :
          this.msgStore['PH0015'].PROTOCOL_NO;
      } else {
        data[7] = dataList[i]['protocolUsage'] = '';
      }
      data[8] = dataList[i]['content'] ? dataList[i]['content'] : '';
  ...
  getTracingCsv(dataList) {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      data[2] = dataList[i]['pharmacistName'];
      data[3] = dataList[i]['patientCode'] ? dataList[i]['patientCode'] : '';
      data[4] = dataList[i]['patientName'] ? dataList[i]['patientName'] : '';
      data[5] = dataList[i]['patientBirthDate'] ? moment(dataList[i]['patientBirthDate']).format('YYYY/MM/DD') : '';
      data[6] = dataList[i]['patientGender'] === 'MALE' ?
        this.msgStore['PHARMACY.TRACING_REPORT'].GENDER.MALE : this.msgStore['PHARMACY.TRACING_REPORT'].GENDER.FEMALE ;
      data[7] = moment(dataList[i]['prescriptionIssueDate']).format('YYYY/MM/DD');
      data[8] = (dataList[i]['compoundingDate']) ?
        moment(dataList[i]['compoundingDate']).format('YYYY/MM/DD') : '';
      data[9] = dataList[i]['content'] ? dataList[i]['content'] : '';
      data[10] = dataList[i]['patientConsent'] ? this.msgStore['PHARMACY.TRACING_REPORT'].PATIENT_CONSENT_YES :
        this.msgStore['PHARMACY.TRACING_REPORT'].PATIENT_CONSENT_NO;
      data[11] = dataList[i]['comments'][0] ? dataList[i]['comments'][0] : '';
      data[12] = dataList[i]['comments'][1] ? dataList[i]['comments'][1] : '';
      exportList.push(data);
    }
    const options = {
      fieldSeparator: ',',
  ...
  goTo(des) {
    ...
    switch (des) {
      case 'drug':
        this.router.navigate(['/ph/ph0006']);
        break;
      case 'protocol':
        this.router.navigate(['/ph/ph0007']);
        break;
    }
  }
  ...
  chooseDetail(topicId) {
    if (this.numTab === 2) {
      this.router.navigate(['ph/ph0003'], {queryParams: {reportId: topicId}});
    }
    if (this.numTab === 3) {
      this.router.navigate(['ph/ph0005'], {queryParams: {topicId: topicId}});
    }
  }
